...
from .config import (
    ...
    get_include_decks, set_include_decks,
)

class ConfigDialog(QDialog):
    def __init__(self, parent: QWidget = None):
        super().__init__(parent)
        self.setWindowTitle("Anki Wiki Sync configuration")
        self.setMinimumWidth(550)

        self.current_path = get_obsidian_path() or ""

        # --- Path Selection Widgets ---
        self.path_label = QLabel("Target folder inside your vault (for example <vault>/anki):")
        ...
        # --- Include Decks List (allowlist; unchecked decks never sync) ---
        self.exclude_label = QLabel("Include Decks in Sync (only checked decks are mirrored):")
        ...
        if mw and mw.col:
            ...
            for deck_name in sorted(mw.col.decks.all_names()):
                ...
                item.setCheckState(checked_state if deck_name in included else unchecked_state)
    ...
    def accept(self):
        ...
        if new_path and os.path.isdir(new_path):
            ...
            if is_vault_root(new_path):
                showWarning(
                    "That folder is an Obsidian vault root. Choose or create a "
                    "dedicated subfolder inside the vault (e.g. 'anki') so the "
                    "sync only ever owns its own quarantined tree."
                )
                return
            set_obsidian_path(new_path)
            
            # Save the deck allowlist (empty list = sync nothing)
            included = []
            try:
                checked_state = Qt.CheckState.Checked
            except AttributeError:
